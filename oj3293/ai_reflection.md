...
| OJ submission ID, if submitted | 639147 |
| OJ status | Pass |
...
```text
I asked Gemini to find the reason of the EOFError.
```
...
```text
OJ grader uses a text file or stream as input; when there are no more lines, Python raises an EOFError. Using while True makes the program keep repeating and breaks after the error.
```
...
```text
I coded everything myself except try-except.
```
