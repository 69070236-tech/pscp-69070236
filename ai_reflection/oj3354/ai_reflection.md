...
| Item | Answer |
|---|---|
| OJ problem number/title | OJ3354 - Hint |
| OJ submission ID, if submitted | 654014 |
| OJ status | Pass |
...
```text
I try to find a way to turn string operators into actual operators.
```
...
```text
There is a module "operator" I can use to achieve that.
```
...
```text
I coded everything myself; I just need to find a way to make it more efficient.
```
...
```text
Operator module. For example ">" = operator.gt
operator.gt(3, 4) = 3 > 4
```
