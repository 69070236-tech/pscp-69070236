...
| Item | Answer |
|---|---|
| OJ problem number/title | OJ3363 - ไข้หวัดกระต่ายสายพันธุ์ใหม่ |
| OJ submission ID, if submitted | 669229 |
| OJ status | Pass |
...
```text
I asked Claude how to continue from my calculation plan.
```
...
```text
First, I use range() to calculate. Claude suggested that it didn't work and gave me a new plan.
Store the value in a list first then use nested loop to calculate each cord  using Chebyshev distance.
```
...
```text
After I know how to do it, I try to fix the code myself.
```
...
```text
In a grid problem, using nested loops is one of the techniques, and distance calculation commonly uses the absolute.
```
