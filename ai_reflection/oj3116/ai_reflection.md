...
| Item | Answer |
|---|---|
| OJ problem number/title | OJ3116 - นวัตกรรมงบประมาณโรงเรียน |
| OJ submission ID, if submitted | 623334 |
| OJ status | Pass |
...
```text
I didn't understand how to create a passkey, so I asked Gemini to clarify. After I understood, I sent my code to Gemini to verify and fix a minor bug.
```
...
```text
I can find the ASCII value of a letter in Python by using ord(). The first step to create a passkey is to find the length of a school name and the ASCII values of the first and last letters. And calculate the code like this: ord() + the place value that is not divisible by 2, and ord() - the place value that is divisible by 2; keep going from 1 to 10.
```
...
```text
After I understood the problem and knew how to find the ASCII, I did everything by myself.
```
...
```text
How to find ASCII values and how to read the problem another way.
```
