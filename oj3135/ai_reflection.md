...
| OJ submission ID, if submitted | 626875 |
| OJ status | Pass |
...
```text
I asked Gemini to help me find the reason for the timeout error.
```
...
```text
The error occurs because of a list. It's been looping too much; if the list has 200,000 items, it will have to loop 200,000^2 times because it's in a loop.
```
...
```text
I coded everything by myself and fixed bugs with Gemini's help.
```
...
```text
A list will loop through all the items, but a set will do it once with a hash function.
```
